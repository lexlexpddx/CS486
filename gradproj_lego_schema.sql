create schema if not exists gp_lego; --gp for grad project

--STOP HERE TO SET YOUR SCHEMA AS DEFAULT BEFORE RUNNING THE REST OF SCRIPT
...
--these are indices OTHER THAN primary keys
--faster searching/querying on those columns
...
create or replace view set_parts
as 
select 
  inventories.set_num,
...
create or replace view part_info
as
select
  part_num,
...
group by part_num, element_id;

create or replace view part_color_info
as
select
  part_num,
  color_id,
...
create or replace view part_nums
as
