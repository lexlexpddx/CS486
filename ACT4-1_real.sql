...
insert into enrollments(enrollment_id, student_id, course_id, semester, grade, grade_value) values
	(1, 1, 201, 'Fall 2024', 'A', 4),
	(2, 1, 202, 'Fall 2024', 'B', 3),
	(3, 2, 202, 'Fall 2024', 'A', 4),
	(4, 3, 201, 'Fall 2024', 'C', 2),
	(5, 3, 203, 'Fall 2024', null, null),
	(6, 4, 204, 'Fall 2024', 'B', 3)
;
...
select
	c.course_name,
	round(avg(e.grade_value), 2) as average_grade
from
	enrollments e
join courses c on e.course_id = c.course_id
where e.grade_value is not null
...
select distinct
	i.name, c.course_name, count(e.student_id) as student_count
from instructors i
join teaches t on i.instructor_id = t.instructor_id
join courses c on t.course_id = c.course_id
...
-- don't forget to reinsert your values for the enrollments table along with grade updates

select distinct
	s.name, c.course_name, i.name
from
	students s
join enrollments e on s.student_id = e.student_id
join courses c on e.course_id = c.course_id
join teaches t on c.course_id = t.course_id and e.semester = t.semester
join instructors i on t.instructor_id = i.instructor_id;

select
	c.course_name as "Course Name", i.name as "Instructor Name", count(distinct e.student_id) as "Number of Students"
from
	courses c
join enrollments e on c.course_id = e.course_id
...
select
	s.name as "Student Name", c.course_name as "Courses NOT taken"
from
 	students s
...
join teaches t on i.instructor_id = t.instructor_id
where
	t.semester != 'Fall 2024';

select
	s.name as "Student Name", s.major as "Major", c.course_name as "Course Name", e.grade as "Grade"
from
	students s
join enrollments e on s.student_id = e.student_id
join courses c on e.course_id = c.course_id
